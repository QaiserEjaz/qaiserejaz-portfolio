...
import { db } from "../firebase"; // Adjust the import path based on your Firebase setup
import {
    SiJavascript,
    SiReact,
    SiReactrouter,
...
const WorkExperience = () => {
    const [certificateUrl, setCertificateUrl] = useState(null); // State to store the fetched certificate URL
    const [isCertificateModalOpen, setIsCertificateModalOpen] = useState(false); // State to control modal visibility
    ...
    const workData = [
        {
            ...
            location: "Remote",
            hasCertificate: false, // Add this field to indicate if a certificate exists
        },
        {
            ...
            location: "Remote",
            hasCertificate: true, // Add this field to indicate if a certificate exists
        },
        {
            ...
            location: "Remote",
            hasCertificate: false, // No certificate for this work experience
        },
        {
    ...
    // Fetch certificate URL from Firebase
    const fetchCertificate = async (position) => {
        try {
            console.log("Fetching certificate for position:", position); // Debugging
            ...
            if (!querySnapshot.empty) {
                ...
                console.log("Certificate URL:", certificateData.Img); // Debugging
                setCertificateUrl(certificateData.Img); // Set the certificate URL
        ...
        <div className="w-full sm:container sm:mx-auto sm:p-6 overflow-hidden">
            ...
            <div className="relative">
                ...
                <div
                    ...
                    style={{ height: `calc(${(workData.length - 1) * 100}% - ${(workData.length - 1) * 72.5}%)` }} // Adjusted height calculation
                ></div>
                ...
                {workData.map((work, index) => (
                    ...
                    >
                        ...
                        <div className="relative bg-gray-900/50 backdrop-blur-lg p-6 rounded-xl border border-white/10 overflow-hidden transition-all duration-300 hover:scale-100 hover:shadow-md group-hover:border-[#6366f1]/50">
                            ...
                            {/* <div className="absolute inset-0 border-0 border-transparent group-hover:border-[#6366f1]/50 rounded-xl transition-all duration-300 pointer-events-none"></div> */}
                            ...
                            {work.project && (
                                <>
                                    <p className="text-white font-semibold mt-2 text-xs md:text-sm">Project: {work.project}</p>
                                    <p className="text-xs md:text-sm text-gray-300">{work.projectDescription}</p>
                                </>
                            )}
